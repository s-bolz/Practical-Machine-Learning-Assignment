...
Our first model has a very high accuracy with 99,9%. Its out-of-bag error rate is 0.07 % which is very good as well. However cross-validation and the out-of-bag error rate only help us to reduce bias but do not protect our model from overfitting. In order to analyze its predicting capabilities on a new data set we use it to predict our testing partition and build a confusion matrix on our prediction.
